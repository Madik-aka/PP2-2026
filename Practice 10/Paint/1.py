...
SCREEN_W     = 900
SCREEN_H     = 650
TOOLBAR_H    = 75                       # увелечение высоты тулбара
...
class PaintApp:
    ...
    def _on_drag(self, pos):
        ...
        if self.tool == PENCIL:
            ...
            pygame.draw.line(self.canvas, WHITE, self.prev_pos, cp, self.eraser_size * 2)
            pygame.draw.circle(self.canvas, WHITE, cp, self.eraser_size)
            self.prev_pos = cp
    ...
    def _draw_shape(self, p1, p2, surface):
        ...
        if self.tool == RECTANGLE:
            ...
            # Рисовка эллипс, вписанный в прямоугольник
            rect = pygame.Rect(min(x1, x2), min(y1, y2), abs(x2 - x1), abs(y2 - y1))
    ...
    def _toolbar_click(self, mx, my):
        ...
        #Координаты кликов 
        # 3. Brush Size (теперь база x = 620)
        if 698 <= mx <= 720 and 6 <= my <= 28:
            self.brush_size = max(1, self.brush_size - 1)
        elif 724 <= mx <= 746 and 6 <= my <= 28:
            self.brush_size = min(40, self.brush_size + 1)

        # 4. Eraser Size (база x = 620)
        if 698 <= mx <= 720 and 36 <= my <= 58:
            self.eraser_size = max(5, self.eraser_size - 2)
        elif 724 <= mx <= 746 and 36 <= my <= 58:
    ...
    def draw(self, surface):
        ...
        #Визуальный курсор на холсте
        if my > CANVAS_TOP:
            if self.tool == PENCIL:
                pygame.draw.circle(surface, self.color, (mx, my), self.brush_size, 1)
            elif self.tool == ERASER:
                pygame.draw.circle(surface, MID_GRY, (mx, my), self.eraser_size, 1)
        ...
        for i, col in enumerate(PALETTE):
            ...
            pygame.draw.rect(surface, BLACK if col == self.color else DK_GRAY, (sx, 6, 32, 28), thick)

        # Кнопки инструментов
        for i, (tool, icon) in enumerate(zip(TOOLS, ICONS)):
            tx = 10 + i * 100
            btn_col = (90, 140, 240) if tool == self.tool else (190, 190, 190)
            pygame.draw.rect(surface, btn_col, (tx, 40, 92, 26), border_radius=4)
            lbl = font.render(icon, True, WHITE if tool == self.tool else BLACK)
            surface.blit(lbl, (tx + 46 - lbl.get_width()//2, 45))

        #положение виджетов (СДВИНУТО ОТ 550 К 620)
        self._draw_size_control(surface, 620, 6,  "Brush", self.brush_size)
        self._draw_size_control(surface, 620, 36, "Eraser",  self.eraser_size)

        # Предпросмотр цвета
        pygame.draw.rect(surface, self.color, (SCREEN_W - 145, 12, 40, 40))
        pygame.draw.rect(surface, BLACK, (SCREEN_W - 145, 12, 40, 40), 2)

        # Кнопка Clear
        pygame.draw.rect(surface, (210, 60, 60), (SCREEN_W - 90, 20, 74, 30), border_radius=5)
        ct = font.render("Clear", True, WHITE)
        surface.blit(ct, (SCREEN_W - 90 + 37 - ct.get_width()//2, 27))
